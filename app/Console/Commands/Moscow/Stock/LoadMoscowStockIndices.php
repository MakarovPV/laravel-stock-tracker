...
use App\Actions\MoscowStockIndexStoreAction;
...
class LoadMoscowStockIndices extends Command
{
     ...
     * @return int
     */
    public function handle(ImoexStock $imoexStock, MoscowStockIndexStoreAction $action)
    {
        ...
        foreach ($moscowStocks as $index => $stockId)
        {
            ...
            foreach ($indices as $val)
            {
                ...
                $action(new MoscowStockIndexDTO(['stock_id' => $stockId, 'index_id' => $indexId[0]]));
            }
        }
    }
}
